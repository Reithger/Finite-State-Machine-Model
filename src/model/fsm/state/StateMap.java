...
//---  Operations   ---------------------------------------------------------------------------
	...
	public void renameStates() {
		...
		for(int i = 0; i < states.size(); i++)
			renameState(stateArr[i].getStateName(), i + "");
		composition = null;
	}
...
//---  Getter Methods   -----------------------------------------------------------------------
	...
	public HashMap<String, ArrayList<String>> getComposedStates(){
		...
		for(String s : composition.keySet()) {
			...
			for(String t : composition.get(s)) {
				nex.add(t);
			}
			out.put(s, nex);
		}
	...
	public Boolean getStateInitial(String nom) {
		return states.get(nom) == null ? null : states.get(nom).getStateInitial();
	}
	
	public Boolean getStateMarked(String nom) {
		return states.get(nom) == null ? null : states.get(nom).getStateMarked();
	}
	
	public Boolean getStateBad(String nom) {
		return states.get(nom) == null ? null : states.get(nom).getStateBad();
	}
	
	public Boolean getStatePrivate(String nom) {
		return states.get(nom) == null ? null : states.get(nom).getStatePrivate();
	}
	
//---  Setter Methods   -----------------------------------------------------------------------

	public void setStateInitial(String nom, boolean in) {
		if(states.get(nom) != null) {
			states.get(nom).setStateInitial(in);
		}
	}

	public void setStateMarked(String nom, boolean in) {
		if(states.get(nom) != null) {
			states.get(nom).setStateMarked(in);
		}
	}

	public void setStatePrivate(String nom, boolean in) {
		if(states.get(nom) != null) {
			states.get(nom).setStatePrivate(in);
		}
	}

	public void setStateBad(String nom, boolean in) {
		if(states.get(nom) != null) {
			states.get(nom).setStateBad(in);
		}
	}
	...
	public void setCompositionStates(HashMap<String, ArrayList<String>> newComposed) {
	...
	public void setStateComposition(String keyState, ArrayList<String> composedStates) {
...
//---  Manipulations - Adding   ---------------------------------------------------------------
	...
	public State addState(State state) {
		...
		if(states.containsKey(stateName))
			return states.get(stateName);
		states.put(stateName, state.copy());
		return states.get(stateName);
	}
	...
	public State addState(State state1, State state2) {
		...
		if(states.containsKey(stateName))
			return states.get(stateName);
		State newState = new State(state1, state2); // TODO: fix the generic types here
